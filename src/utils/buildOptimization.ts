...
export function generateBuildReport(outputDir: string): BuildReport {
  ...
  const assetsInAssetsFolder = assets.filter(
    (a) => a.path.startsWith('assets/') && (a.type === 'css' || a.type === 'js')
  );
  const allAssetsHashed = assetsInAssetsFolder.length === 0 || assetsInAssetsFolder.every((a) => a.hasHash);

  const totalSize = assets.reduce((sum, a) => sum + a.size, 0);
  const avgHtmlSize = htmlFiles.length > 0 ? htmlFiles.reduce((sum, a) => sum + a.size, 0) / htmlFiles.length : 0;
  const avgCssSize = cssFiles.length > 0 ? cssFiles.reduce((sum, a) => sum + a.size, 0) / cssFiles.length : 0;
  const avgJsSize = jsFiles.length > 0 ? jsFiles.reduce((sum, a) => sum + a.size, 0) / jsFiles.length : 0;
