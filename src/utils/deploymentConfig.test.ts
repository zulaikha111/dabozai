...
describe('GitHub Actions Workflow Configuration', () => {
  ...
  it('should run tests before building', () => {
    ...
    // Find test step
    const testStep = buildSteps.find(
      (step) => step.run?.includes('npm test') || step.name?.toLowerCase().includes('test')
    );
  ...
  it('should use actions/deploy-pages for deployment', () => {
    ...
    const deployAction = deploySteps.find((step) => step.uses?.includes('actions/deploy-pages'));
